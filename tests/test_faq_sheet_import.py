"""The return leg of the curated-answer loop — `scripts/faq_import.py`.

Not to be confused with `test_faq_import.py` beside it, which covers reading a
bank's published FAQ *page* into drafts. Two different importers: that one
turns prose into questions, this one reads a translated sheet back. Creating
this file under that name clobbered all twenty-three of its tests, which is
why this one is named the long way round.
